...
- ~~不要把 `SQ_FACTORY_CH1_PSK_HEX` 寫進任何檔案或 commit~~ —— **這條已作廢(2026-07-29)**。
  出廠 PSK 是**公開值**(老闆的決定,出廠映像會公開發布),見根目錄 `CLAUDE.md` §8。
  **不要再把它當機密處理,不要再為此停下來問。**
  `RF-03` 的報告仍然只印長度不印值,但那是**版面與慣例**的選擇,不是保密要求。
